<?php
if (isset($_POST['editpost_submit']) && $_POST['editpost_submit'] == 1) {
    ...
    //print_r($fields);exit;
    //Do New Post Service
    $new_post = edit_post($username, $password, $fields, $overwrite);
...
<script src="<?php echo JS_PATH; ?>/views/editpost_window.js" type="text/javascript"></script>
...
<section class="slideSection slideNewpost">
    ...
    <form role="form" onsubmit="return validateNewPost()" target="_self" name="editpost_submit" id="editpost_submit"
          method="post" enctype="multipart/form-data" action="/editpost">

        <input type="hidden" name="editpost_submit" value="1">
        <input type="hidden" name="postRef" value="<?php echo trim(strip_tags($_REQUEST["postRef"])); ?>">
        <input type="text" class="form-control postTitle" name="title" placeholder="enter a post title" maxlength="42"
               required="required" value="<?php echo trim(strip_tags($_REQUEST["oldtitle"])); ?>">
        <?php if (empty($_REQUEST["oldimage"])) {
            ?>
            <div class="bigButton uploadPhoto">
                <input class="photoUpload" name="imagefile" type='file' accept="image/*"
        ...
        <textarea name="postedTxt" class="form-control counterLimit postText" rows="5" placeholder="enter a post text"
                  maxlength="200">
            <?php echo trim(strip_tags($_REQUEST["oldmessage"])); ?>
        </textarea>
        ...
        <button type="submit" id="submitBtn" name="submit" class="btn btn-primary floatRight clearfix">
